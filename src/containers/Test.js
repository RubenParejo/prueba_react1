...
function Test() {
  ...
  const [responses, setResponses] = useState(contextState.tests.results);
  ...
  const saveResponses = () => {
    setContextState({ tests: { results: { ...responses }, done: true }});
    navigate(`/`);
  };
  
  const resetResponses = () => {
    testPossibleResponses.forEach(response => {
      responsesRef.current.appendChild(document.getElementById(response));
    });
    setResponses({ tests: { done: false }});
  };
      ...
      <div className='bottom-bar exercise-resources'>
        ...
        <div className='exercise-content'>
          <div className="exercise-responses">
            <div ref={responsesRef} id='responses' className="exercise-responses-options">
